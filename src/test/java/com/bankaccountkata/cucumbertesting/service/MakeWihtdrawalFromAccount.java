...
public class MakeWihtdrawalFromAccount extends SpringIntegrationTest {

	CucumberDatas cucumberDatas;

	public MakeWihtdrawalFromAccount(CucumberDatas cucumberDatas) throws Throwable {
		super();
	...
	@Given("The customer name {string} and phone {string} with account type {string} want to withdraw {string}")
	public void the_customer_name_and_phone_with_account_type_want_to_withdraw(String name, String phone,
	...
	public void the_client_calls_wihtdraw() {
	    // Write code here that turns the phrase above into concrete actions
		HttpEntity<CustomerOperationDto> request = new HttpEntity<>(this.cucumberDatas.getCustomerOperationDto());
		OperationDto operationDto = new RestTemplate().postForObject("http://localhost:8080/operations/wihtdraw", request,
				OperationDto.class);
		this.cucumberDatas.setOperationDto(operationDto);
	}

	@Then("the account of client is debited")
	public void the_account_of_client_is_debited() {
	    // Write code here that turns the phrase above into concrete actions
		assertEquals(OperationType.DEBIT, this.cucumberDatas.getOperationDto().getType());
	}
